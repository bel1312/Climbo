class ClimbingGame {
    ...
    generatePlatform(y) {
        ...
        const weights = [40, 20, 15, 15, 10]; // Weighted probability
    ...
    handleInput() {
        ...
        const moveSpeed = this.powerUpActive ? 8 : 6;
        const jumpPower = this.powerUpActive ? 16 : 14;
        
        // Improved left movement with acceleration
        if (this.keys['ArrowLeft'] || this.keys['KeyA']) {
            this.player.velocityX = Math.max(this.player.velocityX - 1.2, -moveSpeed);
        }
        // Improved right movement with acceleration
        else if (this.keys['ArrowRight'] || this.keys['KeyD']) {
            this.player.velocityX = Math.min(this.player.velocityX + 1.2, moveSpeed);
        }
        // Better friction
        else {
    ...
    updatePlayer() {
        ...
        if (-this.cameraY > this.platforms[this.platforms.length - 1].y + 200) {
            for (let i = 0; i < 6; i++) {
                this.generatePlatform(this.platforms[this.platforms.length - 1].y - 120);
            }
        }
    ...
    checkCollisions() {
        ...
        // Obstacle collisions
        this.obstacles.forEach(obstacle => {
            if (this.isColliding(this.player, obstacle) && !this.player.invulnerable) {
                this.takeDamage();
            }
        });
    ...
    respawnPlayer() {
        const nearestPlatform = this.platforms
            .filter(p => p.y > this.cameraY && p.y < this.cameraY + this.canvas.height)
            .sort((a, b) => Math.abs(a.y - this.player.y) - Math.abs(b.y - this.player.y))[0];
        ...
        this.platforms.forEach(platform => {
            if (platform.y > this.cameraY - 50 && platform.y < this.cameraY + this.canvas.height + 50) {
                ...
                if (platform.type === 'bouncy') {
                    ...
                    this.ctx.fillRect(platform.x + 5, platform.y + 5, platform.width - 10, 5);
                }
                ...
                if (platform.type === 'breakable' && platform.health <= 1) {
                    ...
                    for (let i = 0; i < 3; i++) {
                        ...
                        this.ctx.moveTo(x, y);
                        this.ctx.lineTo(x + Math.random() * 20 - 10, y + Math.random() * 10 - 5);
                    }
                    this.ctx.stroke();
                }
            }
    ...
    drawObstacles() {
        this.obstacles.forEach(obstacle => {
            if (obstacle.y > this.cameraY - 50 && obstacle.y < this.cameraY + this.canvas.height + 50) {
                this.ctx.fillStyle = obstacle.type === 'rock' ? '#696969' : 
                                   obstacle.type === 'bird' ? '#8B4513' : '#87CEEB';
                
                if (obstacle.type === 'bird') {
                    // Draw simple bird shape
                    this.ctx.beginPath();
                    this.ctx.ellipse(obstacle.x + 15, obstacle.y + 15, 12, 8, 0, 0, Math.PI * 2);
                    this.ctx.fill();
                } else {
    ...
    drawPowerUps() {
        this.powerUps.forEach(powerUp => {
            if (!powerUp.collected && powerUp.y > this.cameraY - 50 && powerUp.y < this.cameraY + this.canvas.height + 50) {
                ...
                this.ctx.fillText(symbols[powerUp.type] || '?', x + size/2, y + size/2);
            }
        });
    }
    
    drawPlayer() {
        const alpha = this.player.invulnerable && Math.floor(Date.now() / 100) % 2 ? 0.5 : 1;
        ...
        const eyeOffset = this.player.velocityX > 0 ? 1 : this.player.velocityX < 0 ? -1 : 0;
